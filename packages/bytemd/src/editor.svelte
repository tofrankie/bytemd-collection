...
<script lang="ts">
  ...
  import type { BytemdEditorContext, BytemdPlugin, EditorProps as Props } from './types'
  ...
  export let sanitize: Props['sanitize']
  export let remarkRehype: Props['remarkRehype']
  export let mode: NonNullable<Props['mode']> = 'auto'
  export let previewDebounce: NonNullable<Props['previewDebounce']> = 300
  export let placeholder: Props['placeholder']
  export let editorConfig: Props['editorConfig']
  export let locale: Props['locale']
  export let uploadImages: Props['uploadImages']
  export let overridePreview: Props['overridePreview']
  ...
  let editor: BytemdEditorAdapter
  ...
  $: if (editor && plugins) {
    off()
    const cycle = ++pluginCycle
    tick().then(() => {
      if (cycle !== pluginCycle) return
      on()
    })
  }
  ...
  onMount(async () => {
    ...
    const updateBlockPositions = throttle(() => {
      ...
      const leftNodes = hast.children.filter((v): v is Element => v.type === 'element')
      ...
      for (let i = 0; i < leftNodes.length; i++) {
        ...
        const right =
          (rightNode.offsetTop - body.offsetTop) / (previewEl.scrollHeight - previewEl.clientHeight)
    ...
    const editorScrollHandler = () => {
      ...
      const rightRatio =
        ((leftRatio - editPs[startIndex]) * (previewPs[startIndex + 1] - previewPs[startIndex])) /
    ...
    const previewScrollHandler = () => {
      ...
      const leftRatio =
        ((rightRatio - previewPs[startIndex]) * (editPs[startIndex + 1] - editPs[startIndex])) /
    ...
    const handleImages = async (e: Event, itemList: DataTransferItemList | undefined) => {
    ...
    editor.on('drop', async (_, e) => {
      await handleImages(e, e.dataTransfer?.items)
    })
    editor.on('paste', async (_, e) => {
      await handleImages(e, e.clipboardData?.items)
    })
...
>
  <Toolbar
    ...
    on:tab={e => {
      ...
      if (v === 'write') {
        tick().then(() => {
          // https://github.com/pd4d10/bytemd/issues/232
  ...
  <div class="bytemd-body">
    <div class="bytemd-editor" style={styles.edit} bind:this={editorEl}></div>
    ...
    <div class="bytemd-sidebar" class:bytemd-hidden={sidebar === false}>
      ...
      <Help locale={mergedLocale} actions={actions.leftActions} visible={sidebar === 'help'} />
      <Toc
        {hast}
        locale={mergedLocale}
        {currentBlockIndex}
