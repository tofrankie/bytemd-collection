...
- 为包内补充参考 `@tofrankie/bytemd-plugin-highlight` 的 KaTeX 样式导出入口，将构建产物写入 `dist/styles/`，并允许使用者直接从 `bytemd-plugin-math/styles/katex.css` 导入样式，而不是额外依赖 `katex/dist/katex.css` 或 SCSS 入口
