...
- [x] 1.1 审查 `packages/plugin-highlight` 现有样式源码、构建脚本与导出目录，定位所有 `highlight.js` 主题透传逻辑
- [x] 1.2 为 `packages/plugin-highlight` 接入基于 `sass` 的规则编译流程，产出仅包含规则与变量引用的 `styles/pure.css`
...
- [x] 2.1 更新 `packages/plugin-highlight/package.json` 的构建脚本、发布文件清单与 `exports`，暴露 `styles/<theme>.css` 和 `styles/pure.css`
- [x] 2.2 移除或停止发布旧的 `highlight.js` 样式透传产物与 `scss/<theme>` 导出，确保样式契约只指向插件包内 CSS 入口
- [x] 2.3 更新 `packages/plugin-highlight/README.md`，说明新的主题导入方式、`pure.css` 用法、仓库内基准产物角色与主题变量来源

## 3. 验证

- [x] 3.1 运行 `packages/plugin-highlight` 的构建，确认所有主题 CSS 与 `pure.css` 均成功生成并进入发布目录
